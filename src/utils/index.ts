...
export const IVIEW_CSS_TAG = `<link rel="stylesheet" type="text/css" href="//unpkg.com/view-design/dist/styles/iview.css" />`
export const ANIMATECSS_TAG = `<link rel="stylesheet" href="//cdnjs.cloudflare.com/ajax/libs/animate.css/4.1.1/animate.min.css" />`
export const IVIEW_JS_TAG = `<script type="text/javascript" src="//unpkg.com/view-design/dist/iview.min.js"></script>`
...
export const genInjectedCSS = (fromPath, outPath) => {
  ...
  try {
    ...
    let _out = toAbsolutePath(pathJoin(outPath, `/${require('../../package.json').name}/index.css`))
    // 编译less
...
export function genFile(tmplPath: string, outPath: string, data: ReplaceData) {
  ...
  try {
    ...
    console.log('Failure: could not write template!')
  }
